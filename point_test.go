...
func TestCommandPoint(t *testing.T) {
	ctx := context.Background()
	ctx, _ = xray.BeginSegment(ctx, "TestCommandPoint")
	...
	// get a writable point. expect at least one to be writable
	writablePoint := Point{}
	...
	// get all of the points associated with the device under a gateway
	t.Run("comand-point-with-valid-point", func(t *testing.T) {

		err := CommandPointValue(&session, &writablePoint, "true")
		if err != nil {
			t.Fatal("error commanding point: ", err.Error())
		}
